...
export default function Dashboard({ profile, onReset, savedProgress, isDemo, onUnlock, subscription, isBetaUser }) {
  ...
  // NEW: Handles non-toggle course actions from the CourseCard kebab menu.
  // Mutates the active major's courseData.semesters, persists via saveRoadmap,
  // and rebalances completedCourses for removals so progress stats stay coherent.
  // Disabled in combinedView since edits there would be ambiguous (which major?).
  var handleCourseAction = function(si, ci, action) {
    if (combinedView) return;

    var oldSems = (currentProfile.courseData && currentProfile.courseData.semesters) || [];
    var newSems = oldSems.map(function(sem, sIdx) {
      if (sIdx !== si) return sem;
      var courses = (sem.courses || []).slice();
      if (action === 'remove') {
        courses.splice(ci, 1);
        return { ...sem, courses: courses };
      }
      var c = courses[ci];
      if (!c) return sem;
      if (action === 'mark-in-progress') {
        courses[ci] = { ...c, userStatus: 'in-progress', userModified: true };
      } else if (action === 'unmark-in-progress') {
        var copy = { ...c, userModified: true };
        delete copy.userStatus;
        courses[ci] = copy;
      }
      return { ...sem, courses: courses };
    });

    var newCourseData = { ...(currentProfile.courseData || {}), semesters: newSems };
    var newProfile = { ...currentProfile, courseData: newCourseData };

    // For removal: rebalance completedCourses (and courseGrades) so indices stay
    // aligned to the new course array. Higher-indexed courses in the same semester
    // shift down by one; the removed slot is dropped.
    var newCompleted = completedCourses;
    var newGrades = courseGrades;
    if (action === 'remove') {
      newCompleted = {};
      for (var key in completedCourses) {
        if (!completedCourses[key]) continue;
        var parts = key.split('-');
        var keySem = parseInt(parts[0], 10);
        var keyIdx = parseInt(parts[1], 10);
        if (keySem !== si) {
          newCompleted[key] = true;
        } else if (keyIdx < ci) {
          newCompleted[key] = true;
        } else if (keyIdx > ci) {
          newCompleted[keySem + '-' + (keyIdx - 1)] = true;
        }
        // keyIdx === ci: drop it
      }
      newGrades = {};
      for (var gkey in courseGrades) {
        if (!courseGrades[gkey]) continue;
        var gparts = gkey.split('-');
        var gSem = parseInt(gparts[0], 10);
        var gIdx = parseInt(gparts[1], 10);
        if (gSem !== si) {
          newGrades[gkey] = courseGrades[gkey];
        } else if (gIdx < ci) {
          newGrades[gkey] = courseGrades[gkey];
        } else if (gIdx > ci) {
          newGrades[gSem + '-' + (gIdx - 1)] = courseGrades[gkey];
        }
      }
    ...
    // Mirror into majors[] so switching tabs / toggling combined view doesn't
    // restore stale (pre-edit) data.
    setMajors(function(prev) {
      if (!Array.isArray(prev) || prev.length === 0) return prev;
      var next = prev.slice();
      if (next[activeMajorIndex]) next[activeMajorIndex] = newProfile;
      return next;
    });

    // Persist. Fire-and-forget — same pattern as handleMajorSwitch.
    if (user && saveRoadmap) {
      saveRoadmap(newProfile, newCompleted);
    }
  };
  ...
  const handleMajorSwitch = async function(newMajor) {
    ...
    try {
      var res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ schoolName: currentProfile.school, careerPath: currentProfile.careerLabel, majorName: newMajor, customGoal: null, programLevel: currentProfile.programLevel || 'undergraduate', catalogUrl: catalogUrl || '', clubsUrl: clubsUrl || '', priorCoursework: currentProfile.priorCoursework || '' }),
      });
  ...
  const addNewMajor = async function(majorName) {
    ...
    try {
      var res = await fetch('/api/generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ schoolName: currentProfile.school, careerPath: currentProfile.careerLabel, majorName: majorName, customGoal: null, programLevel: currentProfile.programLevel || 'undergraduate', catalogUrl: catalogUrl || '', clubsUrl: clubsUrl || '', priorCoursework: currentProfile.priorCoursework || '' }),
      });
      if (!res.ok) throw new Error('API error');
      var data = await res.json();
      if (data.semesters) {
  ...
  const changeSchool = async function() {
    ...
    try {
      var res = await fetch('/api/generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ schoolName: newSchool, careerPath: currentProfile.careerLabel, majorName: courseData.major, customGoal: null, programLevel: currentProfile.programLevel || 'undergraduate', catalogUrl: catalogUrl || '', clubsUrl: clubsUrl || '', priorCoursework: currentProfile.priorCoursework || '' }),
      });
      if (!res.ok) throw new Error('API error');
      var data = await res.json();
      if (data.semesters) {
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 50, height: 56, background: sidebarBg, borderBottom: '1px solid ' + bdr, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, backdropFilter: 'blur(12px)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            ...
            <div ref={settingsRef} style={{ position: 'relative' }}>
              <button onClick={function() { setSettingsOpen(!settingsOpen); }} style={{ padding: '6px 12px', height: 32, whiteSpace: 'nowrap', borderRadius: 8, border: '1px solid ' + bdr, background: bgCard, color: txSub, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>⋮</button>
              {settingsOpen && (
            ...
            <button onClick={function() { setDarkMode(!darkMode); }} style={{ padding: '6px 12px', height: 32, whiteSpace: 'nowrap', borderRadius: 8, border: '1px solid ' + bdr, background: bgCard, color: tx, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{darkMode ? 'Light mode' : 'Dark mode'}</button>
          </div>
        </header>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                {semesters[activeSemester] && (<>
                  ...
                  <div style={{ display: 'grid', gap: 10 }}>
                    {semesters[activeSemester].courses && semesters[activeSemester].courses.map(function(c, ci) {
                      // Edits only allowed in single-major view (combinedView is a synthesized merged view —
                      // ambiguous which underlying major to save edits to).
                      return <CourseCard
                        key={ci}
                        course={c}
                        semIdx={activeSemester}
                        cIdx={ci}
                        completed={!!completedCourses[activeSemester + '-' + ci]}
                        onToggle={toggleCourse}
                        onAction={combinedView ? undefined : handleCourseAction}
                        accent={accentColor}
                        darkMode={darkMode}
                      />;
                    })}
