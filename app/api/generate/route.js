...
export async function POST(request) {
  try {
    const { schoolName, careerPath, majorName, customGoal, programLevel, catalogUrl, priorCoursework } = await request.json();
    ...
    // NEW: Prior coursework — surfaced to both searches AND the roadmap prompt.
    // The student typed it as free-text; we trust it as-is and let the model do the matching.
    var priorTrimmed = (priorCoursework || '').trim();
    var hasPrior = priorTrimmed.length > 0;
    var priorBlock = hasPrior ? ('\n\nSTUDENT HAS ALREADY COMPLETED THESE COURSES (do NOT re-recommend them):\n' + priorTrimmed + '\n\nWhen choosing courses, treat these as already done — pick complementary or follow-on courses instead. If a prerequisite is on this list, you can move directly to the next course in that sequence.') : '';
