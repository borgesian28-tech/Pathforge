      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 40 }}>
        <div style={{ display: 'flex', background: dm ? '#1a1a1a' : '#f0f0f4', borderRadius: 12, padding: 4, gap: 4 }}>
          ...
          <button onClick={function() { setBilling('annual'); }}
            ...
            Annual
            <span style={{ position: 'absolute', top: -8, right: -8, background: green, color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 6 }}>Save up to 27%</span>
          </button>
        </div>
      </div>
      ...
      <div className="pricing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, maxWidth: 960, margin: '0 auto' }}>
        ...
        <div style={{ padding: '32px 28px', borderRadius: 18, background: bgCard, border: '2px solid ' + accent, position: 'relative', boxShadow: '0 0 40px ' + accent + '15' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 2 }}>
            <span style={{ fontFamily: serif, fontSize: 42, color: tx }}>${billing === 'annual' ? '49' : '5'}</span>
            <span style={{ color: txMut, fontSize: 14 }}>/{billing === 'annual' ? 'year' : 'mo'}</span>
          </div>
          {billing === 'annual' && <p style={{ color: green, fontSize: 12, fontWeight: 600, marginBottom: 6 }}>$4.08/mo — save $11/year</p>}
        ...
        <div style={{ padding: '32px 28px', borderRadius: 18, background: bgCard, border: '1px solid ' + bdr }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 2 }}>
            <span style={{ fontFamily: serif, fontSize: 42, color: tx }}>${billing === 'annual' ? '79' : '9'}</span>
            <span style={{ color: txMut, fontSize: 14 }}>/{billing === 'annual' ? 'year' : 'mo'}</span>
          </div>
          {billing === 'annual' && <p style={{ color: green, fontSize: 12, fontWeight: 600, marginBottom: 6 }}>$6.58/mo — save $29/year</p>}
