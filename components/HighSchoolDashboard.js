      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 50, height: 56, background: headerBg, borderBottom: '1px solid ' + bdr, display: 'flex', alignItems: 'center', padding: '0 20px', gap: 12, backdropFilter: 'blur(12px)' }}>
          ...
          <div ref={settingsRef} style={{ position: 'relative' }}>
            ...
            <button onClick={function() { setDarkMode(!darkMode); }} style={{ padding: '6px 12px', height: 32, whiteSpace: 'nowrap', fontSize: 12, borderRadius: 8, border: '1px solid ' + bdr, background: bgCard, color: tx, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>{darkMode ? 'Light mode' : 'Dark mode'}</button>
          </div>
        </header>
